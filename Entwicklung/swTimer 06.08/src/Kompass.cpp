...
void Winkel::startup()
{
    ...
    chipWrite(ADDRESS1, CTRL2,   0b00100000); //xG
    chipWrite(ADDRESS2, CTRL4_G, 0b10110000); //Full Sclale 2000 dps (degrees per second)

/*

    standardGForce[0] = getAccelX(); 
    standardGForce[1] = getAccelY();
    standardGForce[2] = getAccelZ();*/
...
/*
float Winkel::getRealAccelX()
{
    return standardGForce[0] - getAccelX();
};
float Winkel::getRealAccelY()
{
    return standardGForce[1] - getAccelY();
};
float Winkel::getRealAccelZ()
{
    return standardGForce[2] - getAccelZ();
};
*/
...
accelometer
    ...
    */
    ...
    if (state)
    {
        ...
        data = getGyroZ();
        zahl++;
        //return -1 * (gyroGesWinkelRAW * (millis() - initialTimeCompare) / zahl * 0.001) + passiveDrift();


        //Serial.println(0.98 * (-1 * (gyroGesWinkelRAW * (millis() - initialTimeCompare) / zahl * 0.001) + passiveDrift()) +0.02 * getRealAccelZ());
        //Serial.println("");
        //return 0.98 * (-1 * (gyroGesWinkelRAW * (millis() - initialTimeCompare) / zahl * 0.001) + passiveDrift() ) +0.02 * getRealAccelZ() ;


        Serial.println((atan2(getAccelY(), getAccelZ()) + PI) * RAD_TO_DEG);//x
        Serial.println((atan2(getAccelX(), getAccelZ()) + PI) * RAD_TO_DEG);//y
        Serial.println((atan2(getAccelY(), getAccelX()) + PI) * RAD_TO_DEG);//z
        
        Serial.println("");
        delay(1000);
        /*
        Serial.print("roll: "); Serial.println(getRoll());
        //Serial.print("Gyro: ");Serial.println((-1 * (gyroGesWinkelRAW * (millis() - initialTimeCompare) / zahl * 0.001) /*+ passiveDrift()*///));
        //Serial.println("");
        //*/


         return 0.97 * (-1 * (gyroGesWinkelRAW * (millis() - initialTimeCompare) / zahl * 0.001) /*+ passiveDrift()*/) +0.03 * getYaw();
    }
    else
    {
        data = getGyroZ();
...
float Winkel::DriftgyroBerechnung()
{
    gyroGesWinkelRAW += data;
    data = getGyroZ();
    zahl++;
...
void Winkel::calcUpTime()
{
    upTime = float(millis() - startTime) / 1000.0 / 60.0;
};

float Winkel::passiveDrift()
{
    calcUpTime();//in millisekunden
    return - (passiveDriftPMin * upTime);
};
...
void Winkel::fixPassivDrift()
{
    /*
        ...
        drift rate = ((yaw(t2) - yaw(t1)) / (t2 - t1)) * (180deg / (pi * rad)) * 1min / 60 s
    */
    float rad = 57.29577951 /* 0.017453293 */;
    passiveDriftPMin = 0;
    float yaw1, yaw2;
    float t1, t2;
    yaw1 = DriftgyroBerechnung();
    t1 = millis() / 1000; //umrechnen in sekunden
    delay(10000);
    yaw2 = DriftgyroBerechnung();
    t2 = millis() / 1000; //umrechnen in sekunden
    passiveDriftPMin = ((yaw2 - yaw1) / ((t2) - (t1))) * rad;
    /*
    Serial.print("yaw1: ");Serial.println(yaw1);
    Serial.print("yaw2: ");Serial.println(yaw2);
    Serial.print("t1: ");Serial.println(t1);
    Serial.print("t2: ");Serial.println(t2);
    */
    Serial.println("Abweicung / min: ");Serial.println(passiveDriftPMin);
    state = false;
};
